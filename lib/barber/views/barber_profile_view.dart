...
class _BarberProfileViewState extends State<BarberProfileView>
  ...
  StreamSubscription<DatabaseEvent>? barberSub;

  bool isUpdatingActive = false;
  ProviderServiceModel? barber;
  bool isLoading = true;
  ...
  void _listenToBarber() {
    ...
    if (uid == null) return;
    barberSub = barberDB.child(uid).onValue.listen((event) {
      if (event.snapshot.exists && mounted) {
        final data = Map<String, dynamic>.from(event.snapshot.value as Map);
        setState(() {
          barber = ProviderServiceModel.fromMap(data);
          firstNameController.text = barber!.firstName;
          lastNameController.text = barber!.lastName;
          emailController.text = barber!.email;
          phoneController.text = barber!.phoneNumber;
          addressController.text = barber!.address;
          isLoading = false;
        });
  ...
  Future<void> updateProfile() async {
    ...
    try {
      await barberDB.child(auth.currentUser!.uid).update({
  ...
  Future<void> updateLocation() async {
    try {
      ...
      await barberDB.child(barber!.uid).update({
        'latitude': pos.latitude,
        'longitude': pos.longitude,
      });
      setState(
        () => barber = barber!.copyWith(
          latitude: pos.latitude,
          longitude: pos.longitude,
        ),
      );
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        barber!.isActive ? 'متاح للحجوزات' : 'غير متاح حالياً',
                        style: TextStyle(
                          color: barber!.isActive
                              ? Colors.greenAccent
                              : Colors.redAccent,
                ...
                Switch.adaptive(
                  ...
                  onChanged: isUpdatingActive
                      ...
                      : (val) async {
                          ...
                          setState(() {
                            barber = barber!.copyWith(isActive: val);
  ...
  Widget _editServicesDialog() {
    List<Map<String, dynamic>> editableServices = barber!.services

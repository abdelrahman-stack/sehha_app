...
const _kBgMid = Color(0xFF1B3A5C);
const _kSurf = Color(0xFF152234);
const _kBlue = Color(0xFF1565C0);
const _kBlue2 = Color(0xFF1E88E5);


class BarberRequestsView extends StatefulWidget {
  const BarberRequestsView({super.key});
  @override
  State<BarberRequestsView> createState() => _BarberRequestsViewState();
}

class _BarberRequestsViewState extends State<BarberRequestsView>
    with SingleTickerProviderStateMixin {
  final _auth = FirebaseAuth.instance;
  final _reqDB = FirebaseDatabase.instance.ref('Requests');

  List<BookingAppointmentModel> _requests = [];
  bool _loading = true;
  String _filter = 'All';
  ...
  Future<ClientModel> _fetchPatient(String pid) async {
    final s = await FirebaseDatabase.instance.ref('Clients/$pid').get();
  ...
  Future<void> _adjustTurn(int delta) async {
    ...
    final ref = FirebaseDatabase.instance.ref(
      'Barbers/$uid/dailyTurn/$_todayKey',
    );
    ...
    switch (status) {
      case 'تم القبول':
        await FcmSender.send(
          token: tok,
          title: 'تم قبول حجزك ✂️',
          body: 'الحلاق قبل حجزك – دورك رقم ${b.turnNumber}',
        );
        break;
      case 'تم الرفض':
        ...
        await FcmSender.send(
          token: tok,
          title: 'تم رفض الحجز',
          body: 'نعتذر، الحلاق غير متاح حالياً',
        );
        break;
      case 'تمت الخدمة':
        await FcmSender.send(
          token: tok,
          title: 'قيّم تجربتك ⭐',
          body: 'ساعدنا نحسّن الخدمة بتقييمك',
        );
...
void _showRatingDlg(BookingAppointmentModel b) {
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [
                      ...
                      Colors.amber.withValues(alpha: .04),
                    ],
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          color: rating == 0
                              ...
                              : Colors.amber.withValues(alpha: .85),
                          fontSize: 14,
                  ...
                  Expanded(
                    child: _Btn(
                      ...
                      onTap: () async {
                        ...
                        setState(
                          () => _requests.removeWhere((b) => b.id == id),
                        );
                      },
                    ),
                  ),
  ...
  Widget build(BuildContext context) {
    ...
    final todayCnt = _requests.where((r) => r.date == _todayStr).length;
        ...
        body: Stack(
          children: [
            ...
            const Positioned(
              top: -40,
              right: -40,
              child: _Glow(180, _kBlue, .12),
            ),
            const Positioned(
              bottom: -60,
              left: -40,
              child: _Glow(160, Color(0xFF0D47A1), .12),
            ),
                ...
                children: [
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(
                                color: _kBlue.withValues(alpha: .45),
                                blurRadius: 16,
                              ),
                            ],
                          ),
                  ...
                  SizedBox(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (_, i) {
                        final (label, val, color) = _filterDefs[i];
                        final active = _filter == val;
                        final cnt = val == 'All'
                            ? _requests.length
                            : _requests.where((r) => r.status == val).length;
                        return GestureDetector(
                          onTap: () {
                            HapticFeedback.selectionClick();
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: _kBlue.withValues(alpha: .15)),
          ),
          child: Icon(
            Icons.inbox_rounded,
            color: _kBlue.withValues(alpha: .38),
            size: 34,
          ),
        ),
        const SizedBox(height: 14),
        ...
        Text(
          ...
          style: TextStyle(color: _kBlue2.withValues(alpha: .38), fontSize: 12),
        ),
      ],
    ),
  );
  ...
  Widget _loadingCard() => Container(
    ...
    child: Center(
      child: SizedBox(
        ...
        child: CircularProgressIndicator(
          strokeWidth: 2,
          color: _kBlue2.withValues(alpha: .4),
        ),
      ),
    ),
  );
...
class _BarberRequestCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: _kSurf.withValues(alpha: .88),
        borderRadius: BorderRadius.circular(22),
              ...
              child: Row(
                children: [
                  Icon(
                    ...
                    color: _kBlue2.withValues(alpha: .55),
                    size: 16,
                  ),
...
// HELPERS
// ════════════════════════════════════════════
class _TurnBtn extends StatelessWidget {
  final IconData icon;
  final Color color;
...
class _Btn extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        gradient: primary
            ? const LinearGradient(colors: [_kBlue, _kBlue2])
            : null,
        color: danger

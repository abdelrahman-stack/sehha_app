...
import 'package:sehha_app/views/rating_view.dart';
import '../../core/utils/app_router.dart';
import '../../core/models/provider_service_model.dart';

const _kAccent = Color(0xFF1565C0);
const _kPink = Color(0xFF1E88E5);
const _kDark = Color(0xFF0C0810);

class MaleBarberListView extends StatefulWidget {
  const MaleBarberListView({super.key});
  @override
  State<MaleBarberListView> createState() => _MaleBarberListViewState();
}

class _MaleBarberListViewState extends State<MaleBarberListView> {
  List<ProviderServiceModel> barbers = [];
  ...
  Future<void> checkPendingRating() async {
    ...
    for (final entry in data.entries) {
      ...
      if (map['status'] == 'completed' &&
          map['ratingStatus'] == 'انتظار التقييم') {
        if (!mounted) return;
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (_) =>
                RatingView(bookingId: entry.key, barberId: map['reciver']),
          ),
        );
        break;
      }
  ...
  Future<void> _fetchNearby(double myLat, double myLng) async {
    final event = await FirebaseDatabase.instance.ref('Barbers').once();
  ...
  Widget build(BuildContext context) => Scaffold(
    backgroundColor: _kPink.withValues(alpha: .06),
    body: Stack(
      children: [
        Container(
        ...
        const Positioned(
          bottom: -50,
          left: -40,
          child: _Glow(180, Color(0xFF0D47A1), .05),
        ),
                      ...
                      children: [
                        Column(
                          ...
                          children: [
                            ...
                            Text(
                              'مصففي الشعر القريبين منك في 10 كم',
                              style: TextStyle(
                            ...
                            onTap: () {
                              if (!h.isActive) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: const Text(
                                      'عذرًا، هذا المصفف غير متاح حاليًا',
                                    ),
                              ...
                              GoRouter.of(context).push(
                                AppRouter.kServiceMaleDetailsView,
                                extra: h,
                              );
                            },
                            child: Container(

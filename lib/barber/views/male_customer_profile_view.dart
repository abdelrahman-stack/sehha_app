...
class MaleCustomerProfileView extends StatefulWidget {
  const MaleCustomerProfileView({super.key});
  @override
  State<MaleCustomerProfileView> createState() =>
      _MaleCustomerProfileViewState();
}

class _MaleCustomerProfileViewState extends State<MaleCustomerProfileView>
    with SingleTickerProviderStateMixin {
  final FirebaseAuth auth = FirebaseAuth.instance;
  final DatabaseReference clientsDB = FirebaseDatabase.instance.ref('Clients');
  StreamSubscription<DatabaseEvent>? clientsSub;
  ProviderServiceModel? clients;
  bool isLoading = true;
  ...
  static const _blue2 = Color(0xFF1E88E5);
  ...
  void _listenToClient() {
    ...
    if (uid == null) return;
    clientsSub = clientsDB.child(uid).onValue.listen((event) {
      if (event.snapshot.exists && mounted) {
        final data = Map<String, dynamic>.from(event.snapshot.value as Map);
        setState(() {
          clients = ProviderServiceModel.fromMap(data);
          firstNameController.text = clients!.firstName;
          lastNameController.text = clients!.lastName;
          emailController.text = clients!.email;
          phoneController.text = clients!.phoneNumber;
          addressController.text = clients!.address;
          isLoading = false;
        });
  ...
  Future<void> updateProfile() async {
    ...
    try {
      await clientsDB.child(auth.currentUser!.uid).update({
  ...
  Future<void> updateLocation() async {
    try {
      ...
      await clientsDB.child(clients!.uid).update({
        'latitude': pos.latitude,
        'longitude': pos.longitude,
      });
      setState(
        () => clients = clients!.copyWith(
          latitude: pos.latitude,
          longitude: pos.longitude,
        ),
      );
      ...
      body: Stack(
        children: [
          ...
          const Positioned(
            bottom: -80,
            left: -50,
            child: _Glow(size: 180, color: Color(0xFF0D47A1)),
          ),
    ...
    decoration: BoxDecoration(
      gradient: LinearGradient(
        colors: [
          ...
          const Color(0xFF1565C0).withValues(alpha: .3),
        ],
        begin: Alignment.topCenter,
        end: Alignment.bottomCenter,
      ),
            ...
            Container(
              ...
              child: ClipOval(
                child: pickedImage != null
                    ...
                    : clients!.profileImage.isNotEmpty
                    ? Image.network(clients!.profileImage, fit: BoxFit.cover)
                    : Container(
            ...
            Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.edit_rounded, color: _blue2, size: 20),
                ),

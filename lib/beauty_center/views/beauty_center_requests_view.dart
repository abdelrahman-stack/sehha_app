...
const _accent =  Color(0xFF7EDBD5);
const _ac2 = Color(0xFF1A8A84);
const _pink = Color(0xFFEDD49A);

class BeautyCenterRequestsView extends StatefulWidget {
  const BeautyCenterRequestsView({super.key});
  @override
  State<BeautyCenterRequestsView> createState() =>
      BeautyCenterRequestsViewState();
}

class BeautyCenterRequestsViewState extends State<BeautyCenterRequestsView>
  ...
  Future<ClientModel> _fetchPatient(String pid) async {
    final s = await FirebaseDatabase.instance.ref('Customers/$pid').get();
  ...
  Future<void> _adjustTurn(int delta) async {
    ...
    final ref = FirebaseDatabase.instance.ref(
      'BeautyCenter/$uid/dailyTurn/$_todayKey',
    );
  ...
  Future<void> _updateStatus(BookingAppointmentModel b, String status) async {
    ...
    final tokSnap = await FirebaseDatabase.instance
        .ref('Customers/${b.sender}/fcmToken')
        .get();
    ...
    switch (status) {
      case 'تم القبول':
        await FcmSender.send(
          token: tok,
          title: 'تم قبول حجزك ✅',
          body: 'مركز التجميل قبل حجزك 💅 دورك رقم ${b.turnNumber}',
        );
        break;
      case 'تم الرفض':
        ...
        await FcmSender.send(
          token: tok,
          title: 'تم رفض الحجز',
          body: 'نعتذر، مركز التجميل غير متاح حالياً',
        );
        break;
      case 'تمت الخدمة':
        await FcmSender.send(
          token: tok,
          title: 'تمت الخدمة 💅',
          body: 'نتمنى تكون/ي راضي/ة ❤️ قيّم/ي تجربتك ⭐',
        );
    ...
    for (final item in (snap.value as Map).values) {
      ...
      if (b.turnNumber == cur) {
        await FcmSender.send(
          token: tok,
          title: 'دورك الآن ✨',
          body: 'تفضل/ي، مركز التجميل مستنياكِ 💅',
        );
      }

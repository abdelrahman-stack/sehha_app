...
class FemaleCustomerProfileView extends StatefulWidget {
  const FemaleCustomerProfileView({super.key});
  @override
  State<FemaleCustomerProfileView> createState() =>
      _FemaleCustomerProfileViewState();
}

class _FemaleCustomerProfileViewState extends State<FemaleCustomerProfileView>
    with SingleTickerProviderStateMixin {
  final FirebaseAuth auth = FirebaseAuth.instance;
  final DatabaseReference clientFemalesDB = FirebaseDatabase.instance.ref(
    'ClientFemales',
  );
  StreamSubscription<DatabaseEvent>? clientFemalesSub;
  ProviderServiceModel? clientFemales;
  bool isLoading = true;
  ...
  void _listenToClient() {
    ...
    if (uid == null) return;
    clientFemalesSub = clientFemalesDB.child(uid).onValue.listen((event) {
      if (event.snapshot.exists && mounted) {
        final data = Map<String, dynamic>.from(event.snapshot.value as Map);
        setState(() {
          clientFemales = ProviderServiceModel.fromMap(data);
          firstNameController.text = clientFemales!.firstName;
          lastNameController.text = clientFemales!.lastName;
          emailController.text = clientFemales!.email;
          phoneController.text = clientFemales!.phoneNumber;
          addressController.text = clientFemales!.address;
          isLoading = false;
        });
  ...
  Future<void> updateProfile() async {
    ...
    try {
      await clientFemalesDB.child(auth.currentUser!.uid).update({
  ...
  Future<void> updateLocation() async {
    try {
      ...
      await clientFemalesDB.child(clientFemales!.uid).update({
        'latitude': pos.latitude,
        'longitude': pos.longitude,
      });
      setState(
        () => clientFemales = clientFemales!.copyWith(
          latitude: pos.latitude,
          longitude: pos.longitude,
        ),
      );
                                ...
                                _InfoCard(
                                  children: [
                                    ...
                                    _InfoRow(
                                      ...
                                      label: 'الهاتف',
                                      value: clientFemales!.phoneNumber,
                                      color: _blue2,
                                    ),
                                    _Divider(),
                                    _InfoRow(
  ...
  Widget _buildHeader() => Container(
    ...
    child: Column(
      children: [
            ...
            Container(
              ...
              child: ClipOval(
                child: pickedImage != null
                    ...
                    : clientFemales!.profileImage.isNotEmpty
                    ? Image.network(
                        clientFemales!.profileImage,
                        fit: BoxFit.cover,
                      )
                    : Container(
        ...
        const SizedBox(height: 14),
        Text(
          '${clientFemales!.firstName} ${clientFemales!.lastName}',

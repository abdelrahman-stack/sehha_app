...
import 'package:sehha_app/beauty_salon/views/hairdresser_profile_view.dart';
import 'package:sehha_app/beauty_salon/views/hairdresser_requests_view.dart';
...
class HairdresserView extends StatefulWidget {
  const HairdresserView({super.key});
  @override
  State<HairdresserView> createState() => _HairdresserViewState();
}

class _HairdresserViewState extends State<HairdresserView> {
  int currentIndex = 0;
  void goToStore() => setState(() => currentIndex = 2);
  ...
  Widget build(BuildContext context) {
    final screens = [
      const HairdresserRequestsView(),
      const HairdresserChatListView(),
      StoreView(onGoBackFromOrder: goToStore),
      const SupportAndPaymentView(),
      const HairdresserProfileView(),
    ];

...
class HairdresserProfileView extends StatefulWidget {
  const HairdresserProfileView({super.key});
  @override
  State<HairdresserProfileView> createState() => _HairdresserProfileViewState();
}

class _HairdresserProfileViewState extends State<HairdresserProfileView>
    with SingleTickerProviderStateMixin {
  final FirebaseAuth auth = FirebaseAuth.instance;
  final DatabaseReference hairdresserDB = FirebaseDatabase.instance.ref(
    'Hairdressers',
  );
  StreamSubscription<DatabaseEvent>? hairdresserSub;

  bool isUpdatingActive = false;
  ProviderServiceModel? hairdresser;
  bool isLoading = true;
  ...
  static const _blue = Color(0xFFAD1457);
  static const _blue2 = Color(0xFF880E4F);
  static const _kDark = Color(0xFF0C0810);
  static const _kAccent = Color(0xFFE91E8C);
  ...
  void _listenToBarber() {
    ...
    if (uid == null) return;
    hairdresserSub = hairdresserDB.child(uid).onValue.listen((event) {
      if (event.snapshot.exists && mounted) {
        final data = Map<String, dynamic>.from(event.snapshot.value as Map);
        setState(() {
          hairdresser = ProviderServiceModel.fromMap(data);
          firstNameController.text = hairdresser!.firstName;
          lastNameController.text = hairdresser!.lastName;
          emailController.text = hairdresser!.email;
          phoneController.text = hairdresser!.phoneNumber;
          addressController.text = hairdresser!.address;
          isLoading = false;
        });
  ...
  Future<void> updateProfile() async {
    ...
    try {
      await hairdresserDB.child(auth.currentUser!.uid).update({
  ...
  Future<void> updateLocation() async {
    try {
      ...
      await hairdresserDB.child(hairdresser!.uid).update({
        'latitude': pos.latitude,
        'longitude': pos.longitude,
      });
      setState(
        () => hairdresser = hairdresser!.copyWith(
          latitude: pos.latitude,
          longitude: pos.longitude,
        ),
      );
              ...
              Container(
                ...
                child: ClipOval(
                  child: pickedImage != null
                      ...
                      : hairdresser!.profileImage.isNotEmpty
                      ? Image.network(
                ...
                Switch.adaptive(
                  ...
                  onChanged: isUpdatingActive
                      ...
                      : (val) async {
                          ...
                          setState(() {
                            hairdresser = hairdresser!.copyWith(isActive: val);
  ...
  Widget _editServicesDialog() {
    List<Map<String, dynamic>> editableServices = hairdresser!.services
